...
- **Anthropic path**: `agent/anthropic_provider.py` uses the Claude Agent SDK's `query()` function with a per-request system prompt, `tools=[]` (disables built-in tools from being offered at all — `allowed_tools=[]` alone only skips the permission prompt for offered tools, it doesn't restrict what's offered), `permission_mode="dontAsk"`, and `max_turns=1` — the SDK here is a pure single-turn generation call, not a free-roaming coding agent. Conversation history is composed into the `prompt` string rather than using the SDK's own session/resume mechanism, since our Postgres tables are the source of truth for conversation state.
